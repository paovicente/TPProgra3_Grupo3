...
public class Controlador implements ActionListener
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		} else if (comando.equalsIgnoreCase("IniciarSes"))
		{
			...
			{
				JOptionPane.showMessageDialog(null, "Contrase�a incorrecta"); //Aqu� consideramos que no ser�a correcto que mostrase la contrase�a
				this.vista.cerrar();
				this.setVista(new VentanaLogin());
			}
		...
		} else if (comando.equalsIgnoreCase("RegistrarseAdmin"))
		{
			...
			*/
			...
			if (empleado.getElecciones().getEmps().isEmpty())
				JOptionPane.showMessageDialog(null, "No has realizado ninguna elecci�n!");
			 else {
			...
			if (lista.getEmpleados().isEmpty()) {
				JOptionPane.showMessageDialog(null, "La ronda de encuentros a�n no se ha ejecutado");
				this.vista.cerrar();
				this.setVista(new VentanaEmpleador());
			} else {
	...
	private void activarTicket(Usuario u) { //verificar q no est� activo
		Ticket ticket;
		if (usuario.getCodUsuario()==1) {	//empleado. verificar q no est� activo
